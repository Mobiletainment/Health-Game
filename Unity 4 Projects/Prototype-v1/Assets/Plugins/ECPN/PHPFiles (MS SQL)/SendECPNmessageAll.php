...
if(count($contacts) > 0) {
	foreach($contacts as $contact) {
		$query="SELECT * FROM $tablename WHERE unityID='$contact'";
        $params = array();
        ...
        if(sqlsrv_num_rows($result) > 0) {
            ...
            $regId = sqlsrv_get_field($result,2);   //2 is the row where deviceID is stored
			$os = sqlsrv_get_field($result,3);  //3 is the row where "os" is stored
